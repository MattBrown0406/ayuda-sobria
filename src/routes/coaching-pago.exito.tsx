...
export const Route = createFileRoute("/coaching-pago/exito")({
  head: () => ({
    meta: [{ title: "Pago recibido — Ayuda Sobria" }, { name: "robots", content: "noindex" }],
  }),
...
function SuccessPage() {
  ...
  useEffect(() => {
    if (!token) {
      setStatus("error");
      setMsg("Falta el identificador de la orden.");
      return;
    }
    capture({ data: { orderId: token } })
      .then((r) => {
        if (r.success) {
          ...
          setStatus("error");
          setMsg("El pago aún no se ha completado.");
        }
      })
      .catch((e) => {
        setStatus("error");
  ...
  return (
    <>
      <PageHero
        eyebrow="Coaching"
        title={status === "ok" ? "¡Pago recibido!" : "Procesando pago"}
      />
      <div className="mx-auto max-w-xl px-4 py-12 text-center">
        ...
        {status === "ok" && (
          <>
            <p className="text-lg">
              Gracias. Matt te contactará por correo para coordinar el horario de tu sesión.
            </p>
            <Link
              to="/"
