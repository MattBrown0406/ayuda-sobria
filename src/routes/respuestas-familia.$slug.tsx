  ...
  return (
    <>
      ...
      <Prose>
        ...
        <p>Cada familia es distinta, pero el patrón se repite: la pregunta suele llegar cuando la situación ya lleva tiempo desgastando a todos. La respuesta empieza por bajar el ritmo, ordenar lo que sabes y decidir cuál es el próximo paso <em>de la familia</em>, no del ser querido.</p>
        <h2>Cómo pensarlo con calma</h2>
        <ul>
          <li>Identifica cuánto de la decisión es miedo y cuánto es información.</li>
          <li>Distingue lo urgente (seguridad, dinero, hijos) de lo importante (patrones de largo plazo).</li>
        ...
        <ol className="mt-4 list-decimal pl-6 space-y-2">
          <li>Únete al <Link to="/circulo-familiar">Círculo Familiar</Link> del lunes a las 8:00 PM (PT).</li>
          <li>Si la situación no puede esperar, reserva <Link to="/coaching-familiar">coaching privado</Link>.</li>
          <li>Si hay rechazo repetido al tratamiento o riesgo alto, evalúa la <Link to="/intervencion">intervención</Link>.</li>
        </ol>
        <p>Ver todas las <Link to="/respuestas-familia">preguntas frecuentes de familias</Link>.</p>
      </Prose>
      <CTAStrip />
    </>
  );
