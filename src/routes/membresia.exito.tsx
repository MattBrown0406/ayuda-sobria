...
export const Route = createFileRoute("/membresia/exito")({
  head: () => ({ meta: [{ title: "Membresía activada — Ayuda Sobria" }] }),
...
function SuccessPage() {
    ...
    (async () => {
      for (let i = 0; i < 5; i++) {
        try {
          ...
          if (cancelled) return;
          if (res.success) { setStatus("active"); return; }
        } catch (e) {
          if (i === 4) {
            setStatus("error");
  ...
  return (
    <>
      <PageHero eyebrow="Membresía" title={status === "active" ? "¡Bienvenido!" : "Procesando tu suscripción"} />
      <div className="mx-auto max-w-xl px-4 py-12 text-center">
        ...
        {status === "active" && (
          <>
            ...
            <Link to="/membresia" className="mt-6 inline-block rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Ir a mi membresía</Link>
          </>
        )}
        {status === "pending" && (
