...
function CityPage() {
  ...
  return (
    <>
      <PageHero eyebrow={`${state.name} · Ciudad`} title={`Apoyo familiar en ${city.name}`} description={`Recursos, coaching y reuniones del Círculo Familiar para familias hispanohablantes en ${city.name}, ${state.name}.`} />
      <div className="mx-auto max-w-3xl px-4 py-12">
        ...
        <div className="mt-8">
          <Link to="/apoyo-familiar/$state" params={{ state: state.slug }} className="text-sm font-semibold text-primary hover:underline">← Ver todas las ciudades de {state.name}</Link>
        </div>
      </div>
      <CTAStrip />
    </>
