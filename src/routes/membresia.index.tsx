...
export const Route = createFileRoute("/membresia/")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://ayudasobria.com/membresia" },
    ],
    links: [{ rel: "canonical", href: "https://ayudasobria.com/membresia" }],
  }),
...
function MembresiaPage() {
  ...
  async function handleCancel() {
    ...
    if (!confirm("¿Cancelar tu membresía? Mantendrás acceso hasta la próxima fecha de cobro."))
      return;
    setLoading(true);
    try {
    ...
    <>
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <Prose>
            ...
            <p>
              ...
              (ahorra ~17%). Todos los precios están en dólares estadounidenses (USD). Tu banco o
              PayPal se encargará de la conversión si pagas desde otro país.
            </p>
            <p>
        ...
        <aside className="rounded-lg border border-border bg-card p-6 shadow-sm h-fit">
          ...
          {session === undefined && <p className="mt-4 text-sm text-muted-foreground">Cargando…</p>}
            ...
            <>
              ...
              {membership?.next_billing_date && (
                <p className="mt-2 text-xs text-muted-foreground">
                  Próximo cobro:{" "}
                  {new Date(membership.next_billing_date).toLocaleDateString("es-ES")}
                </p>
              )}
              <button
            ...
            <>
              {membership?.status === "cancelled" && membership.access_ends_at && (
                <p className="mt-4 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-900">
                  Membresía cancelada. Acceso hasta{" "}
                  {new Date(membership.access_ends_at).toLocaleDateString("es-ES")}.
                </p>
              )}
              <button
